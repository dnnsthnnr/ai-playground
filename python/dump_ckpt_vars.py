...
def main(checkpoint_file, output_dir, remove_variables_regex=""):
    chk_fpath = os.path.expanduser(checkpoint_file)
    reader = tf.train.NewCheckpointReader(chk_fpath)
    var_to_shape_map = reader.get_variable_to_shape_map()
    output_dir = os.path.expanduser(output_dir)
    tf.gfile.MakeDirs(output_dir)
    manifest = {}
    remove_vars_compiled_re = re.compile(remove_variables_regex)

    var_filenames_strs = []
    for name in var_to_shape_map:
        if (remove_variables_regex and
...
if __name__ == '__main__':
    ...
    if unparsed:
        ...
        exit(-1)
    main(FLAGS.checkpoint_file, FLAGS.output_dir, FLAGS.remove_variables_regex)
